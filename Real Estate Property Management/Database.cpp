...
{
	String^ constring="datasource=127.0.0.1;port=3306;user=root;password=rajasthan"; // Defining Connection. Change the name of database, port, username and password before using
...
bool Database::login_verify(String^ user, String^ pass) // Function to check the login details.
		  {
			  if(user=="" || pass=="")
				  return false;
			  String^ s="SELECT * from real_estate.users where username='"+user+"' and password='"+pass+"';";//Change the table name from "Real_Estate.Users" to "YOUR_DATBASE.YOUR_TABLE"
	 ...
	 {
		 String^ s="INSERT into real_estate.users (`username`,`password`,`name`,`email`,`contact`,`location`) VALUES ('"+user+"','"+pass+"','"+name+"','"+email+"','"+contact+"','"+location+"')";
		 MySqlCommand^ cmd=gcnew MySqlCommand(s,connection);
		 try
		 {
			 cmd->Connection->Open();
	 ...
	 void Database:: properties(array<String^>^ columns) // Function to add entries to the properties table. Send an array of Strings as parameter. If some column is to be left blank, then send "" instead.
	 {
		 ...
		 String^ s="INSERT into real_estate.properties(";
		 for(int i=0;i<15;i++)
		 {
			 s=s+"`"+names[i]+"`";
			 if(i!=14)
	 ...
	 {														// This Function returns an Int32 array containing the ids of the resultant rows.	
		 array<String^>^ names={"address","city","area","price","sell_rent","image_path","floor_plan","map","username","bhk","type","decription","floor_number","total_floors","age"};
		 String^ query="SELECT SQL_CALC_FOUND_ROWS * From real_estate.properties WHERE ";
		int count=0;
